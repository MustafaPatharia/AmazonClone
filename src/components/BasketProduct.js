...
function BasketProduct({id,title,price,hasPrime,image, quantity, deliveryDate}) {
  ...
  return (
    <div className="grid grid-cols-4 lg:grid-cols-[160px,minmax(400px,1fr),140px,160px,50px] grid-flow-row-dense gap-x-8 gap-y-2 border-gray-100 border-b-2 py-6">
        ...
        <div className="flex flex-col space-y-2  col-span-3 lg:col-span-1">
            ...
            { hasPrime ? (
            <div className="hidden lg:flex items-center space-x-2 text-xs md:text-sm text-gray-500">
            ...
            <div className="hidden lg:flex flex-col md:flex-row md:items-center md:space-x-2 text-xs md:text-sm text-gray-500">
            ...
            <div className="lg:hidden font-semibold text-2xl">
                <Currency quantity={price*quantity} currency="INR"/>
            </div>

            <div className="hidden md:flex items-center space-x-3 lg:space-x-5 text-amazon_yellow text-xs md:text-sm">
                ...
                <div className="h-4 w-0.5 bg-amazon_yellow hidden lg:block"></div>
                <p className="link hidden lg:block">See more like this</p>
            </div>
        </div>
        ...
        <div className="col-span-3 lg:col-span-1 flex items-center">
            { hasPrime ? (
            <div className="flex lg:hidden items-center space-x-2 text-xs md:text-sm text-gray-500">
                <img loading="lazy" className="w-10 h-6" src="/images/amazon_prime.png" alt="amazon_prime" />
                <p>Get it <span className="font-semibold">{deliveryDate}</span> FREE delivery by Amazon</p>
            </div> )  : (
            <div className="flex lg:hidden md:flex-row items-center space-x-2 text-xs md:text-sm text-gray-500">
                <p>Get it <span className="font-semibold">{deliveryDate}</span> FREE delivery on your first order shiped by Amazon</p>
            </div>
            )}
            {/* Price */}
            <div className="hidden lg:flex mx-auto font-semibold text-xl">
                <Currency quantity={price*quantity} currency="AED"/>
            </div>
        </div>

        {/* 5th Grid Column */}
        <div className="flex w-full items-center mx-auto col-span-full lg:col-span-1 ">
            <div onClick={removeItemFromBasket} className="lg:block text-gray-400 cursor-pointer hidden">
            ...
            <div className="flex w-full text-center space-x-4 text-white md:hidden">
                <button className="rounded-md w-full p-3 bg-amazon_yellow">Delete</button>
